...
const Dashboard = ({ userId: propUserId }) => {
  ...
  const [actionToast, setActionToast] = useState(null) // { type: 'success'|'error', message: string }
  const showActionToast = (type, message) => {
  ...
  useEffect(() => {
    const handleCreateJobPosting = async (event) => {
      ...
      try {
        ...
        showActionToast('success', '🎉 Job posting created successfully!')
        // Refresh recruiter dashboard jobs
        try { window.dispatchEvent(new CustomEvent('#sym:refreshJobs')) } catch { }
      } catch (err) {
        console.error('createJobPosting failed', err)
        showActionToast('error', '❌ Failed to create job posting. Please try again.')
      } finally {
  ...
  return (
    <div className={`h-screen p-3 text-gray-900 font-sans flex flex-col overflow-auto`} style={{ background: 'linear-gradient(135deg, var(--background), rgba(0,119,181,0.03))', ...(isRecruiter ? { ['--primary']: '#0077B5', ['--secondary']: '#005885', ['--primary-foreground']: '#ffffff', ['--background']: '#f0f8ff', ['--foreground']: '#003d5c', ['--card']: '#e6f7ff', ['--card-foreground']: '#003d5c', ['--border']: 'rgba(0,61,92,0.06)', ['--ring']: 'rgba(0,119,181,0.15)' } : {}) }}>
      ...
      <ToastPortal toasts={toasts} />
      {/* Action toast (job creation, etc.) */}
      {actionToast && createPortal(
        <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 2147483647, pointerEvents: 'none' }}>
          <div style={{
            display: 'inline-flex', alignItems: 'center', gap: 10,
            padding: '12px 22px', borderRadius: 14,
            background: actionToast.type === 'success' ? 'linear-gradient(135deg,#1C1A2E,#2D2A44)' : 'linear-gradient(135deg,#2D1018,#3D0F1C)',
            boxShadow: actionToast.type === 'success' ? '0 8px 32px rgba(110,215,165,0.25),0 2px 8px rgba(0,0,0,0.4)' : '0 8px 32px rgba(220,20,60,0.25),0 2px 8px rgba(0,0,0,0.4)',
            border: `1px solid ${actionToast.type === 'success' ? 'rgba(110,215,165,0.3)' : 'rgba(220,20,60,0.3)'}`,
            color: '#fff', fontWeight: 600, fontSize: 14, letterSpacing: 0.1,
            animation: 'toastSlideUp 0.3s ease'
          }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: actionToast.type === 'success' ? '#6ED7A5' : '#dc143c', flexShrink: 0 }} />
            {actionToast.message}
          </div>
        </div>,
        document.body
      ...
      <style>{`@keyframes toastSlideUp { from { opacity:0; transform:translateY(16px) } to { opacity:1; transform:translateY(0) } }`}</style>
      <div className="w-full h-[100vh] flex-1 flex flex-col" style={{ perspective: 900 }}>
        ...
        <header className="mb-2 flex-shrink-0" ref={(el) => { window.__dashboard_header_el = el }}>
          <div className={`flex items-center justify-between p-3 rounded-xl shadow-sm border ${isRecruiter ? 'bg-white border-[#E4DFF5]' : 'bg-white border-transparent'}`}>
            <div className="flex items-center justify-center gap-3 flex-shrink-0">
              <div>
                <h1
                  className={`text-lg md:text-xl font-semibold truncate mb-0 ${isRecruiter ? 'text-[#1C1A2E]' : 'text-gray-900'}`}
            ...
            <nav className="hidden md:flex items-center gap-2 flex-shrink-0">
              <div className="inline-flex items-center gap-2 bg-transparent p-1 rounded-full">
                {!isRecruiter ? (
                  <>
                    {[
                      { key: 'candidate', label: 'Recommendations', icon: '✨', onClick: () => { setCandidateInitialTab('jobs'); setCurrentView('candidate') } },
                      { key: 'saved', label: 'Saved', icon: '⭐', onClick: () => { setCandidateInitialTab('jobs'); setCurrentView('saved') } },
                      { key: 'chat', label: 'Chat', icon: '💬', onClick: () => { setCandidateInitialTab('jobs'); setCurrentView('chat') } },
                      { key: 'resume', label: 'Resume', icon: '📄', onClick: () => { setCurrentView('resume'); setTimeout(() => { try { window.dispatchEvent(new CustomEvent('app:openResume')) } catch { } }, 100) } },
                    ].map(({ key, label, icon, onClick }) => (
                      <button
                        key={key}
                        onClick={onClick}
                        className={`flex items-center gap-1.5 px-3 md:px-4 py-1.5 md:py-2 rounded-full text-sm font-semibold transition-all ${currentView === key
                          ? 'shadow-sm text-white'
                          : 'text-[#6E6B86] hover:bg-[#E4DFF5] hover:text-[#9A8CF2]'
                          }`}
                        style={currentView === key ? { background: 'linear-gradient(135deg, #9A8CF2, #6ED7A5)', color: '#fff' } : {}}
            ...
            <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
              ...
              <button
                ...
                className={`md:hidden p-2 rounded-full transition-colors duration-200 touch-manipulation ${isRecruiter ? 'hover:bg-[#E4DFF5] text-[#9A8CF2]' : 'hover:bg-gray-100 text-gray-700'}`}
              ...
              <div className="relative" ref={avatarMenuRef}>
                ...
                {avatarMenuOpen && (
                  <div className="hidden md:block absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                    ...
                    <div className="py-1.5">
                      <button
                        onClick={() => { setCurrentView('settings'); setAvatarMenuOpen(false) }}
                      ...
                      <button
                        onClick={() => { navigate(`/attitude-test?role=${isRecruiter ? 'recruiter' : 'candidate'}`); setAvatarMenuOpen(false) }}
                        className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-green-50 hover:text-green-700 transition-colors"
                      >
                        ...
                        <span className="font-medium">Attitude Test</span>
                      </button>
                    </div>

                    <div className="border-t border-gray-100 py-1.5">
                      <button
                        onClick={() => { apiLogout(); setAvatarMenuOpen(false) }}
                ...
                {avatarMenuOpen && (
                  <div className="md:hidden absolute right-0 top-full mt-2 w-64 bg-white rounded-md shadow-lg border border-gray-100 overflow-hidden z-50">
                    ...
                    <div className="p-4 border-b border-gray-200">
                      <div className="flex flex-col gap-2">
                        {!isRecruiter ? (
                          <>
                            {[
                              { key: 'candidate', label: '✨ Recommendations', onClick: () => { setCandidateInitialTab('jobs'); setCurrentView('candidate'); setAvatarMenuOpen(false) } },
                              { key: 'saved', label: '⭐ Saved Jobs', onClick: () => { setCandidateInitialTab('jobs'); setCurrentView('saved'); setAvatarMenuOpen(false) } },
                              { key: 'chat', label: '💬 Chat', onClick: () => { setCandidateInitialTab('jobs'); setCurrentView('chat'); setAvatarMenuOpen(false) } },
                              { key: 'resume', label: '📄 Resume', onClick: () => { setCurrentView('resume'); setAvatarMenuOpen(false); setTimeout(() => { try { window.dispatchEvent(new CustomEvent('app:openResume')) } catch { } }, 100) } },
                            ].map(({ key, label, onClick }) => (
                              <button
                                key={key}
                                onClick={onClick}
                                className={`w-full text-left px-4 py-3 rounded-xl font-semibold transition-all ${currentView === key
                                  ? 'text-white'
                                  : 'text-[#6E6B86] hover:bg-[#E4DFF5] hover:text-[#9A8CF2]'
                                  }`}
                                style={currentView === key ? { background: 'linear-gradient(135deg, #9A8CF2, #6ED7A5)' } : {}}
                              >
                                {label}
                              </button>
                            ))}
                          ...
                          <>
                            {[
                              ...
                              <button
                                key={key}
                                onClick={() => { setCurrentView(key); setAvatarMenuOpen(false) }}
                                className={`w-full text-left px-4 py-3 rounded-xl font-semibold transition-all ${currentView === key
                                  ? 'text-white'
                                  : 'text-[#6E6B86] hover:bg-[#E4DFF5] hover:text-[#9A8CF2]'
                                  }`}
                    ...
                    <div className="p-4 space-y-2">
                      ...
                      <button onClick={() => { setCurrentView('settings'); setAvatarMenuOpen(false) }} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-green-50 hover:text-green-700 rounded-xl transition-colors">
                        <svg className="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                        <span className="font-medium">Settings</span>
                      </button>
                      <button onClick={() => { navigate(`/attitude-test?role=${isRecruiter ? 'recruiter' : 'candidate'}`); setAvatarMenuOpen(false) }} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-green-50 hover:text-green-700 rounded-xl transition-colors">
                        <svg className="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
                        <span className="font-medium">Attitude Test</span>
                      </button>
                      <div className="border-t border-gray-100 mt-1 pt-1">
                        <button onClick={() => { apiLogout(); setAvatarMenuOpen(false) }} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-red-600 hover:bg-red-50 rounded-xl transition-colors">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
                          <span className="font-medium">Logout</span>
                        </button>
                      </div>
      ...
      {createJobOpen && (
        <JobPostingForm recruiterId={userId} onClose={() => setCreateJobOpen(false)} onSubmit={handleJobPostingSubmit} externalSubmitting={jobCreating} />
      )}
